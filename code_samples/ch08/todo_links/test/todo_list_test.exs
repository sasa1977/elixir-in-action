defmodule TodoListTest do
  ...
  test "add_entry" do
    ...
    assert(2 == sample_todo_list() |> Todo.List.entries(~D[2013-12-19]) |> length)
    assert(1 == sample_todo_list() |> Todo.List.entries(~D[2013-12-20]) |> length)
    assert(0 == sample_todo_list() |> Todo.List.entries(~D[2013-12-22]) |> length)

    assert(2 == shopping_entry().id)
    assert(~D[2013-12-20] == shopping_entry().date)
  ...
  test "delete_entry" do
    ...
    assert(0 == deleted_list |> Todo.List.entries(~D[2013-12-20]) |> length)
  end

  defp sample_todo_list do
    Todo.List.new
    |> Todo.List.add_entry(%{date: ~D[2013-12-19], title: "Dentist"})
    |> Todo.List.add_entry(%{date: ~D[2013-12-20], title: "Shopping"})
    |> Todo.List.add_entry(%{date: ~D[2013-12-19], title: "Movies"})
  end

  defp shopping_entry(todo_list \\ sample_todo_list()) do
    todo_list
    |> Todo.List.entries(~D[2013-12-20])
    |> Enum.at(0)
  end
end
